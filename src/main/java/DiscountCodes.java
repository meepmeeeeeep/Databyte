...
public class DiscountCodes extends JPanel {
    public DiscountCodes() {
        ...
        //---- discountCodesButton ----
        Image discountCodesBg = new ImageIcon(Objects.requireNonNull(getClass().getResource("/assets/images/discountCodesButtonActive.png"))).getImage();
        ...
        JButton[] buttonsToRestrict = restrictedButtons.getOrDefault(userRole, new JButton[]{
                dashboardButton, userManagementButton, financialsButton, resupplyButton, discountCodesButton
        });
    ...
    private void dashboard(ActionEvent e) {
        // Open Dashboard
        SwingUtilities.getWindowAncestor(this).dispose(); // Close Discount Codes
    ...
    private void create(ActionEvent e) {
        JFrame frame = new JFrame("Create Discount Code");
        frame.setContentPane(new CreateDiscountCodeForm(this));
        frame.pack();
        frame.setLocationRelativeTo(null);
    ...
    private void edit(ActionEvent e) {
        ...
        if (selectedRow != -1) {
            ...
            JFrame frame = new JFrame("Edit Discount Code");
            frame.setContentPane(new EditDiscountCodeForm(code, this));
            frame.pack();
            frame.setLocationRelativeTo(null);
            frame.setResizable(false);
            frame.setVisible(true);
        } else {
            JOptionPane.showMessageDialog(this, "Please select a discount code to edit.");
        }
    }
    // Hover Effects - Mouse Enter
    ...
    private void initComponents() {
        ...
        //======== windowTitleContainer ========
        {
            ...
            //---- dashboardLabel ----
            dashboardLabel.setText("Discount Codes");
        ...
        //======== scrollPane1 ========
        {
            ...
            //---- discountCodesTable ----
            discountCodesTable.setRowHeight(40);
            discountCodesTable.setFont(new Font("Segoe UI", Font.PLAIN, 14));
            scrollPane1.setViewportView(discountCodesTable);
        }
    ...
    private void setTableTheme() {
        // Use Custom Theme for Discount Codes Table
        discountCodesTable.setShowGrid(false);
        DefaultTableCellRenderer customRenderer = new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value,
                ...
                if (value != null) {
                    if (column == 1) { // Discount percentage
                        label.setText(String.format("%.2f%%", (Double)value));
                    } else if (column == 7) { // Minimum purchase
                        label.setText(String.format("%.2f", (Double)value));
                    }
                }
        ...
        for (int i = 0; i < discountCodesTable.getColumnCount(); i++) {
            discountCodesTable.getColumnModel().getColumn(i).setCellRenderer(customRenderer);
        }

        // Set Table Header Style
        JTableHeader header = discountCodesTable.getTableHeader();
        ...
        int[] preferredWidths = {120, 100, 120, 120, 100, 100, 80, 120};
        int[] minWidths = {100, 80, 100, 100, 80, 80, 60, 100};

        for (int i = 0; i < preferredWidths.length; i++) {
            TableColumn column = discountCodesTable.getColumnModel().getColumn(i);
            column.setPreferredWidth(preferredWidths[i]);
            column.setMinWidth(minWidths[i]);
        }

        // Lock Column Re-order
        discountCodesTable.getTableHeader().setReorderingAllowed(false);
    }
}
